...
function changeColor() {
    sayHelloButton.style = "background-color: black; color: white;";
    // sayHelloButton.classList.add("button-style");
}
...
function changeColor2() {
    sayHelloButton.style = "background-color: #EFEFEF; color: black;";
}
    ...
    axios.get("http://localhost:3000/animals")
        .then(response => {
            for(let i = 0; i < response.data.length; i++) {
                ...
                newAnimal.textContent = response.data[i];
...
// CODE HERE
function queryTest() {
    axios.get("http://localhost:3000/query-test?key=test")
    .then(response => {
        console.log(response.data)
    });
}
